...
Cara pakai dasar
---------------
1. Jalankan dari parent folder project (penting — jangan `cd tocket` lalu `python3 main.py`).
   - Karena Python mencari package dari current directory; jalankan dari folder yang berisi folder `Tocket`.
2. Pertama kali dijalankan:
...
Troubleshooting cepat
---------------------
- Error "No module named tocket.__main__" or script failing if run from inside `tocket/`:
...
- List Repositori kosong:
  - Pastikan token valid dan punya scope `repo`. Tes di REPL:
    ```py
...
2) Buat repo & upload file:
```
[1] Create Repositori -> isi nama
...
Selamat menggunakan Tocket. Selalu utamakan keamanan & kenyamanan anda setiap penggunaan tool Tocket, terima kasih.
